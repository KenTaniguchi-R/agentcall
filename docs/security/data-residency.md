...
D1 is the durable identity and audit system of record. Current token and recovery
secrets, plus retired roster join/admin secrets, are stored only as SHA-256 hashes; hashes are still
...
|---|---|---|
| `handles` | Organization, handle, admin/member role, online-token hash, recovery-proof hash/generation/redemption time, agent kind, and creation time. Direct identity, authority, and authentication verifiers; personal data. | Indefinite. Token rotation replaces the online hash; recovery issue/redeem replaces its proof hash. Handle release/deletion is not implemented. |
...
| `org_events` | Organization-invite, call-lifecycle, credential-recovery, retention-policy, and legal-hold administration action; organization; actor and target identities/types; granted target role where applicable; source IP/country; description; and time. Recovery rows contain only a public proof ID, never a proof/hash/token. Call rows identify caller/callee addresses and a call ID but exclude prompt/response bodies. Security audit evidence and personal data. | The newest 10,000 events per organization are retained. Invite, recovery, and audit-control mutations trim in their D1 batch; call transitions use an idempotent Durable Object outbox and trim on D1 delivery. Invite-row cleanup does not otherwise delete audit evidence. Admin-only checkpointed export is supported; configured time-based/legal retention does not yet trigger expiry. |
| `cards` | Handle, agent description/type, task catalogue/examples/keywords, per-caller blocks, and update time. User-authored content plus relationship policy; potentially confidential and personal. | Indefinite, with an upsert replacing the prior card. No delete path exists. |
...
| `rosters` | Retired roster IDs, organizations, admin-secret hashes, creation times, and audit-budget counters. Legacy organization and authentication data retained because D1 migrations are append-only. | No current route reads, writes, or deletes these rows. Existing rows remain until a future approved erasure migration. |
| `roster_join_keys` | Retired roster key prefixes, hashes, descriptions, issuer handles, and lifecycle times. Legacy authentication, provenance, and personal data. | No current route reads, writes, or deletes these rows. Existing rows remain until a future approved erasure migration. |
| `roster_members` | Retired roster membership and admission provenance. Legacy personal relationship data. | No current route reads, writes, or deletes these rows. Existing rows remain until a future approved erasure migration. |
| `roster_events` | Historical append-only roster mutation evidence: roster/organization, actor and target identities/types, source IP/country, description, and time. Security audit evidence and personal data. | Indefinite. No current route creates roster events; export and retention-readiness continue to read the historical ledger. |
...
| `recovery_receipts` | Organization, handle, recovery generation, operation/public credential IDs, secret digests, and commit time for exact lost-response replay. Raw recovery proofs and client tokens are never stored. | Replay authorization expires after seven days. Expired rows are deleted opportunistically by later recovery operations; no scheduled cleanup exists yet, so physical retention can be longer. |
| `recovery_evictions` | Organization, handle, recovery generation, retry count, and next/last attempt times for idempotent post-recovery socket eviction. No credential material. | Deleted after the identity Durable Object confirms eviction; otherwise retried with bounded exponential backoff by the five-minute operations alarm. |
...
|---|---|---|
| WebSocket state and serialized attachments | Live listener/caller sockets; organization, caller/callee handles, source IP/country when available, call ID, and test timeout. Personal/relationship metadata. Message and outcome content passes only as signed HPKE ciphertext. | Socket lifetime; attachments support hibernation and disappear with the socket. |
| `call:*` plus `queue:*`, `due:*`, `dedupe:*`, lease and quota indexes | Call ID, organization, caller/callee stable identities and credential generation, correlation/message IDs, deadlines, timestamps, native/A2A lifecycle state, lease metadata, sizes, and opaque encrypted request/outcome envelopes. Context IDs, reply text, peer failure codes/details, and offered tasks are not relay-visible fields. Personal activity metadata plus bounded ciphertext. | Synchronous records retain the six-minute deadline. Owner-enabled durable request/result ciphertext is retained for at most 72 hours; expired ciphertext is deleted and minimal terminal/dedupe metadata remains for 24 more hours. Due indexes drive alarm cleanup; delayed alarms can extend physical deletion beyond the logical deadline. |
...
organizations, handles, or operation prefixes, depending on the
...
The native `CARD_RL` and `READ_RL` bindings are a third, Cloudflare-managed
counter surface. Their keys include IP addresses and their configured window is 60
...
| Workers invocation/custom/error logs | Request metadata can contain handle route parameters. The status-telemetry error contains only error class and whether the D1 health counter was recorded; it never contains tenant, subject, outcome, card body, or raw error. | Workers Logs retain up to 3 days on Free or 7 days on Paid when enabled. This repository does not declare observability, sampling, Logpush, or a destination, so dashboard/account state must be verified separately. |
| Call content | Caller messages, task/context identifiers, replies, and peer failure content traverse the Worker, `HandleDO`, and WebSockets only as signed HPKE ciphertext. An opaque request and outcome may be written to `call:*` for owner-enabled durable delivery. A2A exposes the outcome only as an encrypted raw HPKE artifact. | Ciphertext has the bounded `call:*` retention above and is not written to D1, Analytics Engine, or console logs. The relay still processes and can log routing, lifecycle, network, timing, and size metadata; endpoint-local plaintext is outside Cloudflare residency controls. |
...
context bindings, the encrypted outbound-job binding store, retained HPKE key
epochs, the execution journal, `calls.log`, `tools.log`, and task/work directories
