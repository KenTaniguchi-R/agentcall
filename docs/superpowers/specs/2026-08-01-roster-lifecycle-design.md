...
**Not in scope:** a retention policy for `roster_events`, and any endpoint that reads
it. The rows exist for the relay operator, who has D1 access. #47 (abuse monitoring)
is the issue that should decide both.
...
because every statement here is known up front and none depends on a prior read.)
...
Every affected member self-cleans on their next `search` — worst case one 15-minute
`CACHE_TTL_MS`. The client half of expulsion was built before the server half
existed; this phase lights it up without touching it.
...
- **The enumeration invariant, asserted on bodies rather than status codes**:
  unknown roster, wrong join secret, wrong admin secret, and non-member produce
  byte-identical responses. This is the test that makes `guards.ts` load-bearing
