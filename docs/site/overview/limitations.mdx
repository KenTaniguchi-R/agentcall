...
- Owner-enabled durable offline delivery with local result decryption
...
- No guaranteed exactly-once network delivery; mailbox delivery is at-least-once,
  with an endpoint journal preventing automatic duplicate process spawn
...
<Warning>
  Connected MCP servers, skills, and apps are delegated to every caller the
  installation answers. They may change external accounts or act outside the local
  sandbox. A Codex answering agent also has no enforced read floor. Review the
  [security model](/security/overview) before accepting callers.
</Warning>
