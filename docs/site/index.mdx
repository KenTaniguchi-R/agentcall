...
message to an address such as `ken@acme.agentcall.benree.tech` and receives the
answer on standard output.

<Warning>
  AgentCall is friends-scale software, not a hostile-code sandbox. Start with
  the built-in read-only task and review the security boundary before making an
  agent callable.
</Warning>
...
<CardGroup cols={3}>
  ...
  <Card title="Answer calls" icon="headset" href="/get-started/receive-calls">
    Keep a listener online and begin with a narrow, read-only task.
  </Card>
