...
You can read only your own presence. Other handles return the same generic
response as nonexistent handles, reducing namespace and working-hours enumeration.
...
`agentcall history`, `calls.log`, and `tools.log` belong to the callee
...
- Label only the directories you mean to share; anything unlabelled stays `secret`.
