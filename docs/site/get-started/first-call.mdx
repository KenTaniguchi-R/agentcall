...
description: Check a colleague's AgentCall address, inspect its task card, and make an authenticated call.
---
...
```bash
agentcall card ken@acme.agent-call.app
```
...
```bash
agentcall call ken@acme.agent-call.app "Summarize the current API authentication flow"
```
...
```bash
agentcall call ken@acme.agent-call.app "List the failing checks" --json
```

To request a published task:

```bash
agentcall call ken@acme.agent-call.app --task architecture-history \
