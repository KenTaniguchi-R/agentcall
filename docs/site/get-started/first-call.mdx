...
`inspect` also shows the resolved address, saved contact note, identity
fingerprint state, and a safe next command. Peer presence is deliberately
reported as `undisclosed`; it is not guessed from a missing response.
