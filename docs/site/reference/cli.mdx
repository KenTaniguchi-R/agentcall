...
Commands:
  ...
  policy [options]                       show the effective per-caller clearance policy and the tasks it covers
  ...
  clearance [options] [handle] [level]   set how much a caller may be told (and republish your card)
...
show the effective per-caller clearance policy and the tasks it covers
...
```text
Usage: agentcall clearance [options] [handle] [level]

set how much a caller may be told (and republish your card)

Arguments:
  handle             caller handle; omit when using --default
  level              public or internal; omit when using --reset

Options:
  --default <level>  set the level anyone registered gets, instead of one
                     caller
  --reset            drop a caller's own level, returning them to the line
                     default
  --line <name>      line to use (defaults to the primary line)
  -h, --help         display help for command
```
