...
| Agent answers "I can't reach that" | The path is unlabelled, so it classifies `secret` | Label it in `sensitivity.json`, or check `agentcall doctor` for a source that has moved |
