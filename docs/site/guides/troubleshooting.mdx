...
| Agent answers "I can't reach that" | The path is outside every root, or on the denylist | Add a root in `scope.json`, or check `agentcall doctor` for a root that has moved |
...
`history` summarizes the installation's `calls.log` and guarded tool attempts. It is local
