...
or `~/AgentCall/<line>/public/` only as a fallback when none is usable. It is
the same for every caller the line answers.
...
Run `agentcall doctor` and confirm, for every callable line, that it reports a
