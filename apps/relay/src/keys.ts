...
type StoredIdentity = { identity_pub: string; relay_origin: string; address: string };

async function storedIdentity(
  c: Context<RelayAppEnv>, org: string, handle: string,
): Promise<StoredIdentity | null> {
  const row = await c.env.DB.prepare(
    "SELECT identity_pub, relay_origin, address FROM identity_keys WHERE org = ? AND handle = ?",
  ).bind(org, handle).first<StoredIdentity>();
  return row ?? null;
}
...
export function mountKeys(app: Hono<RelayAppEnv>): void {
  ...
  app.put("/v1/keys/identity", rateLimit(NATIVE_CARD, "identity"), async (c) => {
    ...
    //
    // relay_origin and address are stored exactly as submitted (#345) — GET
    // below serves them back verbatim instead of reconstructing relay_origin
    // from whatever host the read request happens to arrive on, which is not
    // guaranteed to be the host that was actually signed.
    const inserted = await c.env.DB.prepare(
      "INSERT INTO identity_keys (org, handle, identity_pub, relay_origin, address, created_at) " +
        "VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(org, handle) DO NOTHING",
    ).bind(
      identity.org, identity.handle, parsed.data.identity_pub,
      parsed.data.relay_origin, parsed.data.address, Date.now(),
    ).run();
    if ((inserted.meta.changes ?? 0) === 1) return c.json({ ok: true });

    // Something was already there. Re-publishing the identical record is
    // idempotent — a CLI retrying after a network timeout must not hard-fail.
    // A different relay_origin for the same identity_pub is a different signed
    // transcript, not a retry, and falls through to the same 409 as a
    // different key.
    const existing = await storedIdentity(c, identity.org, identity.handle);
    return existing?.identity_pub === parsed.data.identity_pub &&
      existing.relay_origin === parsed.data.relay_origin && existing.address === parsed.data.address
      ? c.json({ ok: true })
  ...
  app.put("/v1/keys/encryption", rateLimit(NATIVE_CARD, "identity"), async (c) => {
    ...
    const publisher = await storedIdentity(c, identity.org, identity.handle);
    if (publisher === null) return c.json({ error: "publish an identity key first" }, 409);
    ...
    try {
      identityKey = await importIdentityPublicKey(publisher.identity_pub);
    } catch {
    ...
    //
    // relay_origin and address are stored exactly as submitted (#345), same as
    // the identity row above.
    const r = parsed.data;
    const inserted = await c.env.DB.prepare(
      "INSERT INTO encryption_keys " +
        "(org, handle, key_id, suite, pub, epoch, not_before, not_after, prev, relay_origin, address, signature, created_at) " +
      ...
      r.not_before, r.not_after, r.prev, r.relay_origin, r.address, body.signature, Date.now(),
    ...
    if ((inserted.meta.changes ?? 0) !== 1) {
      ...
      const existing = await c.env.DB.prepare(
        "SELECT key_id, suite, pub, epoch, not_before, not_after, prev, relay_origin, address, signature " +
        ...
        not_before: number; not_after: number; prev: string | null;
        relay_origin: string; address: string; signature: string;
      }>();
      if (
        ...
        existing.prev === r.prev && existing.relay_origin === r.relay_origin &&
        existing.address === r.address && existing.signature === body.signature
      ) {
  ...
  app.get("/v1/keys/:handle", rateLimit(NATIVE_READ, "ip"), async (c) => {
    ...
    const publisher = await storedIdentity(c, identity.org, target);
    if (publisher === null) return c.json(NOT_FOUND, 404);
    ...
    // Served exactly as stored (#345) — not reconstructed from the live
    // request's own host, which is not guaranteed to be the host that was
    // actually signed (self-hosted behind a reverse proxy, an IP-based
    // deployment, or local dev all break that assumption).
    return c.json({
      identity: {
        v: 1, relay_origin: publisher.relay_origin, address: publisher.address,
        identity_pub: publisher.identity_pub,
      },
      encryption: {
        record: {
          v: 1, relay_origin: row.relay_origin, address: row.address,
